  ...
  return (
    <header className="room-header glass-card">
      <div className="room-header-left">
        <h2 className="room-title">JAMSC</h2>
        <div className="room-code-wrapper">
          <span className="room-code-label">Mã phòng:</span>
          <span className="room-code">{room?.id ?? '------'}</span>
          <button className="btn-icon btn-copy" onClick={handleCopyCode} title="Sao chép mã phòng">
      ...
      <div className="room-header-right">
        <div className={`connection-status${connected ? '' : ' disconnected'}`}>
          ...
          <span className="status-text">{connected ? 'Đã kết nối' : 'Mất kết nối'}</span>
        </div>
